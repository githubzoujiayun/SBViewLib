...
public class ActionSheet extends Fragment implements OnClickListener {
	...
	//------------
	/**
	 * 显示action，只能在FragmentActivity中显示，必带cancel按钮，需要指定菜单回调
	 * 其中theme是样式, ios6和ios7是两个内置的样式，可以自己定义：
	 ...
	 * @param listener
	 */
	public static void showActionSheet(FragmentActivity activity, String[] titles, String cancelBtn, int themeId, ActionSheetListener listener)
	{
		activity.setTheme(themeId);
		ActionSheet.createBuilder(activity, activity.getSupportFragmentManager())
			.setCancelButtonTitle(cancelBtn)
			.setOtherButtonTitles(titles)
			.setCancelableOnTouchOutside(true).setListener(listener).show();
	}
	
	public static void showActionSheetLikeIOS6(FragmentActivity activity, String[] titles, String cancelBtn, ActionSheetListener listener)
	{
		showActionSheet(activity, titles, cancelBtn, R.style.ActionSheetStyleIOS6, listener);
	}
	public static void showActionSheetLikeIOS7(FragmentActivity activity, String[] titles, String cancelBtn, ActionSheetListener listener)
	{
		showActionSheet(activity, titles, cancelBtn, R.style.ActionSheetStyleIOS7, listener);
	}
